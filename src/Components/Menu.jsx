  ...
  return (
    <div>
      ...
      {isOpen && (
        <nav
          ref={ref}
          className="absolute flex flex-col justify-around   border border-brown/20  right-7 shadow-lg top-10 w-40 h-40 bg-cream z-50 rounded-lg"
        >
          {navLink.map((link) => {
            return (
              <Link
                onClick={TogglerHandler}
                key={link.id}
                className="flex flex-row mr-5 gap-2  items-center  text-brown "
                href={link.path}
              >
